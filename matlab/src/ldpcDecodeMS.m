% ldpcDecodeMS    LDPC decode LLR data with minimum-sum algorithm.
%
% Calling syntax:
%     [y, numIter] = ldpcDecodeMS(x, pcm, maxIter, earlyExit)
%
% Input:
...
function [y, numIter] = ldpcDecodeMS(x, pcm, maxIter, earlyExit)
...
for iter = 1:maxIter
    ...
    for nn = 1:nz
        ...
        if (vLqAbsMinIdx(rowsnn) == nn)
            vLr(nn) = vLr(nn) * vLqAbsMin2(rowsnn);
        else
            vLr(nn) = vLr(nn) * vLqAbsMin(rowsnn);
        end
    end
